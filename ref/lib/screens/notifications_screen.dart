...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _loadUserAndAnnouncements() async {
    try {
      ...
      if (user?.id != null) {
        ...
        print('DEBUG: Current user ID from AuthRepository: $_currentUserId (${user.username})');
  ...
  Future<void> _loadAnnouncements() async {
    ...
    try {
      ...
      // Lấy tất cả thông báo (bao gồm cả inactive) để debug
      final allAnnouncements = await _repo.getAllAnnouncements();
      print('DEBUG: Found ${allAnnouncements.length} total announcements in database');
      
      // Lấy thông báo cho user (chỉ active)
      final announcements = await _repo.getAllAnnouncementsForUser(_currentUserId!);
      print('DEBUG: Found ${announcements.length} active announcements for user');
      
      // Debug: In ra chi tiết các thông báo
      for (final announcement in announcements) {
        print('DEBUG: Announcement ${announcement.id}: "${announcement.title}" (active: ${announcement.isActive})');
      }
      ...
      for (final announcement in announcements) {
        if (announcement.id != null) {
          final isRead = await _repo.isAnnouncementReadByUser(_currentUserId!, announcement.id!);
          if (isRead) {
      ...
      setState(() {
        ...
        _unreadCount = announcements.where((a) => 
          a.id != null && !readIds.contains(a.id)).length;
        _isLoading = false;
      });
      print('DEBUG: Loading completed. Announcements: ${_announcements.length}, Unread: $_unreadCount');
        ...
        setState(() {
          _announcements = [
            Announcement(
              ...
              content: 'Đây là thông báo mẫu để kiểm tra giao diện. Bạn có thể xóa thông báo này.',
            ...
            Announcement(
              ...
              content: 'Ứng dụng đã được cập nhật với nhiều tính năng mới. Hãy khám phá các chức năng trong menu.',
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildDetailRow('Ngày tạo', _formatDateTime(announcement.createdAt)),
              _buildDetailRow('Trạng thái', isRead ? 'Đã đọc' : 'Chưa đọc', 
                valueColor: isRead ? Colors.grey.shade600 : Colors.red.shade600),
            ],
          ),
        ),
        actions: [
