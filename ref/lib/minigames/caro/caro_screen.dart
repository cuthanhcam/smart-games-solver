...
class _GameScreenState extends State<GameScreen> {
  ...
  void _makeAIMove() async {
    ...
    final move = AIService.getBestMoveWithDifficultyForPlayer(gameState, widget.difficulty, currentPlayer);
    print('AI move for ${currentPlayer.name}: $move'); // Debug logging
    print('Current board state before move: ${gameState.board[0][0]}'); // Debug logging

    if (mounted) {
        setState(() {
          if (move[0] != -1 && move[1] != -1) {
            print('Making AI move at: ${move[0]}, ${move[1]} for ${currentPlayer.name}'); // Debug logging
            final moveSuccess = gameState.makeMove(move[0], move[1], currentPlayer);
            print('AI move success: $moveSuccess'); // Debug logging
            print('Board after move: ${gameState.board[move[0]][move[1]]}'); // Debug logging
            print('Game over: ${gameState.gameOver}, Winner: ${gameState.winner}'); // Debug logging
              ...
              children: [
                ...
                Text(
                  gameState.winner == null 
                      ...
                      : widget.gameMode == GameMode.evE
                      ? 'Chiến thắng!'
                      : (gameState.winner == Player.x ? 'Thắng cuộc!' : 'Thua cuộc!'),
                ...
                Text(
                  gameState.winner == null
                      ? 'Trận đấu hòa'
                      : widget.gameMode == GameMode.evE
                      ? (gameState.winner == Player.x ? 'Máy 1 thắng!' : 'Máy 2 thắng!')
                      : (gameState.winner == Player.x ? 'Bạn đã thắng!' : 'Máy đã thắng!'),
                  style: const TextStyle(
                    fontSize: 18,
                      ...
                      Row(
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              Text(
                                widget.gameMode == GameMode.evE ? 'Máy 1' : _currentUsername,
                                style: const TextStyle(
                                  color: Colors.white, 
                                  fontSize: 16,
                                  fontWeight: FontWeight.bold,
                              ...
                              Text(
                                widget.gameMode == GameMode.evE ? 'AI Tấn công' : 'Người chơi',
                      ...
                      Row(
                        children: [
                          Column(
                            ...
                            children: [
                              Text(
                                widget.gameMode == GameMode.evE ? 'Máy 2' : 'Máy',
                                style: const TextStyle(
                                  color: Colors.white, 
                                  fontSize: 16,
                                  fontWeight: FontWeight.bold,
                              ...
                              const SizedBox(height: 4),
                              Text(
                                widget.gameMode == GameMode.evE ? 'AI Phòng thủ' : '${_difficultyLabel(widget.difficulty)}',
                                style: const TextStyle(color: Colors.white70, fontSize: 12),
                                overflow: TextOverflow.ellipsis,
                              ),
                            ],
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Icon(
                      isAIThinking ? Icons.hourglass_empty : Icons.lightbulb_outline,
                      color: Colors.white,
                      size: 32,
                    ),
                  ),
