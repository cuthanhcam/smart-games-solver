...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> _loadUsers() async {
    ...
    try {
      ...
      final userCount = users.where((user) => !user.isAdmin).length;
      ...
      setState(() {
        _users = sortedUsers;
        _totalUsers = users.length; // Total = admin + user
  ...
  Future<void> _deleteUser(AppUser user) async {
    ...
    if (currentUser?.id == user.id) {
      _showErrorSnackBar('Không thể xóa chính mình');
      return;
    }
  ...
  Future<void> _toggleAdminStatus(AppUser user) async {
    ...
    final currentUser = await _repo.getCurrentUser();
    if (currentUser?.id == user.id) {
      _showErrorSnackBar('Không thể thay đổi quyền của chính mình');
      return;
    }

    try {
      final success = await _repo.updateUserAdminStatus(
        user.id!,
        !user.isAdmin,
      );
      if (success) {
  ...
  Future<void> _showBanDialog(AppUser user) async {
    ...
    // Không thể cấm chính mình
    final currentUser = await _repo.getCurrentUser();
    if (currentUser?.id == user.id) {
      _showErrorSnackBar('Không thể cấm chính mình');
      return;
    }
          ...
          children: [
            ...
            Container(
              ...
              child: ListTile(
                ...
                onTap: () =>
                    Navigator.of(context).pop(const Duration(minutes: 1)),
              ),
            ),
            ...
            Container(
              ...
              child: ListTile(
                ...
                onTap: () => Navigator.of(
                  context,
                ).pop(const Duration(days: 365 * 100)), // 100 years
              ),
            ),
          ],
        ),
      ...
      try {
        ...
        if (success) {
          _showSuccessSnackBar(
            'Đã cấm user "${user.username}" trong ${_formatDuration(banDuration)}',
          );
                    ...
                    Row(
                      children: [
                        ...
                        if (user.bannedUntil != null &&
                            DateTime.now().isBefore(user.bannedUntil!)) ...[
                          const SizedBox(width: 8),
                          Container(
                            padding: const EdgeInsets.symmetric(
                              horizontal: 8,
