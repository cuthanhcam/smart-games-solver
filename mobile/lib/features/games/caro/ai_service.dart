...
class AIService {
  ...
  static List<int> getBestMoveWithDifficulty(GameState gameState, Difficulty difficulty) {
    return getBestMoveWithDifficultyForPlayer(gameState, difficulty, gameState.currentPlayer);
  }

  /// Lấy nước đi tốt nhất với độ khó cho player cụ thể (hỗ trợ AI vs AI)
  static List<int> getBestMoveWithDifficultyForPlayer(GameState gameState, Difficulty difficulty, Player player) {
    try {
          ...
          case Difficulty.normal:
            ...
            if (_aiAdapter.canWinNext(gameState)) {
              ...
              if (defensiveMove[0] != -1) {
                ...
                move = _aiAdapter.getBestMove(gameState); // Always use best move
              }
            }
            break;
      ...
      if (move[0] >= 0 && move[0] < gameState.board.length &&
          move[1] >= 0 && move[1] < gameState.board[0].length &&
  ...
  static List<int> _getAggressiveMove(GameState gameState, Difficulty difficulty) {
    try {
  ...
  static List<int> _getDefensiveMove(GameState gameState, Difficulty difficulty) {
    try {
  ...
  static List<int> _findBestAttackMove(GameState gameState) {
    // Tìm nước đi tạo ra chuỗi dài nhất có thể
    int bestScore = -1;
    ...
    for (int row = 0; row < gameState.board.length; row++) {
      for (int col = 0; col < gameState.board[row].length; col++) {
        if (gameState.board[row][col] == CellState.empty) {
          int score = _evaluateMove(gameState, row, col, gameState.currentPlayer);
          if (score > bestScore) {
            bestScore = score;
            bestMove = [row, col];
          }
  ...
  static int _evaluateMove(GameState gameState, int row, int col, Player player) {
    CellState cellState = player == Player.x ? CellState.x : CellState.o;
    int score = 0;
    ...
    for (var dir in directions) {
      ...
      for (int i = 1; i < 5; i++) {
        int r = row + dir[0] * i;
        int c = col + dir[1] * i;
        if (!gameState.isValidCell(r, c) || gameState.board[r][c] != cellState) break;
        count++;
      }
      ...
      if (count >= 5) score += 1000; // Thắng
      else if (count == 4) score += 100; // Gần thắng
      else if (count == 3) score += 10; // Chuỗi 3
      else if (count == 2) score += 1; // Chuỗi 2
    }
